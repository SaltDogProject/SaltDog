...
async function google_translate(text) {
    text = (0, formatter_js_1.normalizeText)(text);
    // const targetUrl = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=at&dt=bd&dt=ex&dt=ss&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&q=${encodeURI(
    //     text
    // )}`;
    const targetUrl = `https://translate.googleapis.com/translate_a/single?client=gtx&dt=at&dt=bd&dt=ex&dt=ss&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dj=1&ie=UTF-8&sl=auto&tl=zh_CN&q=${encodeURI(text)}`;
    //const targetUrl = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=t&q=google`;
    console.log('URL', targetUrl);
    const req = await axios_1.default.get(targetUrl, {
        headers: {
