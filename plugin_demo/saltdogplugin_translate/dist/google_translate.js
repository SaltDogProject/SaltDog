...
async function google_translate(text) {
    ...
    const req = await axios_1.default.get(`https://translate.google.cn/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=at&dt=bd&dt=ex&dt=ss&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&q=${encodeURI(text)}`, {
        headers: {
    ...
    if (rawData.length == 0)
        throw new Error('No translate response data');
    ...
    speech['src'] = isWord ? rawData[0][rawData[0].length - 1][2] : null,
        speech['dst'] = rawData[0][rawData[0].length - 1][3];
    if (!isWord)
        return {
            speech,
            paragraph,
