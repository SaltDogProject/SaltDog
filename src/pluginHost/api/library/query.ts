...
export function listDir(dirID: number): Promise<IDirList> {
    return invokeLibraryMethodAsync('listDir', dirID) as Promise<IDirList>;
}
export function listLib(): Promise<ILibList[]> {
    return invokeLibraryMethodAsync('listLib') as Promise<ILibList[]>;
}
export function mkdir(parentDirID: number, dirname: string) {
    return invokeLibraryMethodAsync('mkdir', parentDirID, dirname);
}
