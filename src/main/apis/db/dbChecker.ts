...
const configFilePath = path.join(STORE_PATH, 'settings.json');
const configFileBackupPath = path.join(STORE_PATH, 'settings.bak.json');
...
function dbPathDir(): string {
    return path.dirname(dbPathChecker());
}

export { dbChecker, dbPathChecker, dbPathDir };
